...
`science-research-core` 将生命科学扩展到化学、物理、作物/植物、生态、动物
生理、生物化学、分子生物学和医学/转化研究设计，同时保持实验执行和临床决策门禁。

Linux 是所有 profile 可复用的执行基础层，不是一个学科包。
`biology-research-core` 将生物学能力扩展到计算分析之外，连接课题构思、文献
检索与筛选、证据综合、实验设计、统计规划、湿实验治理和论文交付。
