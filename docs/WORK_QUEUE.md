...
| `MIG-005` | done | UI and presentation skills: `frontend-ui-ux`, `frontend-philosophy`, `ai-slop-remover`, and slide/document helpers. | `UI_PRESENTATION_PROFILE_REVIEW.md` records provenance gaps, overlap with existing frontend rules, and the decision not to add a redundant first-party skill. | Keep frontend behavior in repository-native rules and audited HTML profiles. |
